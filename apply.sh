...
original_name="user_management_svc"
original_urlname="user-management-svc"
original_description="Awesome user_management_svc created by Stanxxy" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/user_management_svc "${name}"
...
then
    ...
    echo "then run 'user_management_svc' or 'python -m user_management_svc'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'user_management_svc' or 'python -m user_management_svc' or 'poetry run user_management_svc'"
fi
